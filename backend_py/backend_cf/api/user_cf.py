...
def run_analyze(user_id, rate, phone):
    ...
    # num of user
    print(
        f"\njumlah user dataframe rate: {rate['user_id'].nunique()} unique user")

    # num of merk
    print(
        f"jumlah merek dataframe rate: {rate['merk_id'].nunique()} unique merek")

    # num of ratings
    print(
        f"jumlah rating dataframe rate: {rate['rating'].nunique()} unique rating")

    # list of unique rating (list rating pada rating.csv): 0,1,2,3,4,5
    print(
        f"list rating pada dataframe rate: {sorted(rate['rating'].unique())}\n")
    ...
    print('hasil aggregate:\n', agg_rate, '\n')

    # print(agg_rate.info())

    # filter brand dengan jumlah rating > 5
    ...
    # num of user
    print(
        f"\njumlah user dataframe rate: {df_gt['user_id'].nunique()} unique user")

    # num of movies
    print(
        f"jumlah merek dataframe rate: {df_gt['merk_id'].nunique()} unique merk")

    # num of ratings
    print(
        f"jumlah rating dataframe rate: {df_gt['rating'].nunique()} unique rating")

    # list of unique rating
    print(
        f"list rating pada dataframe rate: {sorted(df_gt['rating'].unique())}\n")

    # 3 create user_brand matrix (matrik rating user terhadap merek2)
    ...
    print(f'picked_user = {picked_user_id}\n user sim:\n', sim_user)
    sim_user.info()

    # print out top n sim user
    # print(f"user yang mirip dengan {picked_user_id}:\n{sim_user}")

    # convert sim user into dictionary, untuk di return
    ...
    print('hasil picked_user_id_brand_used:\n', picked_user_id_brand_used)
    ...
    # ubah bentuk dictionary ke dataframe pandas
